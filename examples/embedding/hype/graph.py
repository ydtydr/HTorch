...
def reconstruction_worker(adj, lt, objects, progress=False):
    ...
    for object in tqdm(objects) if progress else objects:
        ...
        dists = lt.Hdist(lt[None, object]).squeeze(-1)
        dists[object] = 1e12
...
def eval_reconstruction(adj, lt, workers=1, progress=False):
    '''
    ...
    if workers > 1:
        ...
        results = reconstruction_worker(adj, lt, objects, progress)
